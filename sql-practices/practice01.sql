...
-- 문제3.
-- 여직원과 남직원은 각 각 몇 명이나 있나요?(group by로)
select gender, count(gender)
from employees
group by gender;

-- 문제3-1. 
-- 여직원과 남직원은 각 각 몇 명이나 있나요?(if문으로)
...
select count(salary) as '급여가 120,000 이상인 사원 수'
from salaries
where salary > 120000;
...
select count(emp_no) as 'Engineer 사원 수'
from titles
group by title
